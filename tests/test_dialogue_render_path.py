...
class ShotStillTests(unittest.TestCase):
    def test_parser_keeps_shot_framing(self):
        from pipeline.llm import _scene_mode_lines
        mode, lines = _scene_mode_lines({"mode": "dialogue", "lines": [
            {"speaker": "K", "text": "Hi", "shot": "close-up of K at the bar"},
            {"speaker": "J", "text": "Yo"},
        ]})
        self.assertEqual(lines[0]["shot"], "close-up of K at the bar")
        self.assertNotIn("shot", lines[1])

    def test_clean_lines_keeps_shot(self):
        import webapp.backend.main as m
        ls = m._clean_lines([{"speaker": "K", "text": "Hi", "shot": " tight close-up "},
                             {"speaker": "J", "text": "  "}])
