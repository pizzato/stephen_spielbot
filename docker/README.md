...
| `COMFYUI_REF` | `v0.37.0` | Pin ComfyUI to a tag/branch/commit for reproducible workers. `make install` writes this pin. v0.37.0 is what this repo ships: it includes Qwen-Image 2.1, and it is past the Music 3 (v0.33.0), LTX 2.5 (v0.32.0) and H3 (v0.30.0) floors. `make start` rebuilds a worker whose image is older |
...
| `H3_MOTION_CONTEXT_REF` | `5335715a` (pack 0.6.2) | H3 clip-chaining nodes behind **Settings → Styles → Chained scenes**, plus this repo's audio-track node. Pinned. 0.6.2 matches ComfyUI ≥ 0.34 and does not patch it; an older pin fails every singing clip on the current image. Set **empty** to leave the nodes out of the image |
