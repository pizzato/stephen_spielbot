...
export default function Publish({ initialWorkDir, go }) {
  ...
  // Burn the cover into the video's opening — Shorts ignore uploaded
  ...
  const burnFirstFrame = async () => {
    ...
    try {
      const { task_id } = await api.firstFrameCover({
        work_dir: workDir, seconds: ffSeconds,
      })
      await new Promise((resolve, reject) => {
        const check = async () => {
          try {
        ...
        <Card className="reveal reveal-d2">
          ...
          <div className="mt-16">
            <Field label="Cover phrase" hint="The short text drawn on the cover — follows the title until you edit it. Only words wrapped in *asterisks* get the accent colour (the title's default marks them for you — remove them for no accent). Press Enter to force a line break.">
              <textarea className="textarea" rows={2} value={coverPhrase} maxLength={80} disabled={!workDir || busy === 'phrase'}
          ...
          <div className="mt-16" style={{ borderTop: '1px solid var(--line)', paddingTop: 14 }}>
            ...
            <p className="muted" style={{ fontSize: 11.5, marginTop: 6 }}>
              For Shorts, burn the cover into the opening of the film before publishing —
              the feed picks its own frame, not the thumbnail. Nothing is prepended, so
              the timing never shifts.
            </p>
            <div className="row center gap-8 mt-10 row--wrap">
              ...
              <Button variant="ghost" icon="image"
                disabled={!workDir || busy === 'burn' || !finalUrl || !coverUrl}
