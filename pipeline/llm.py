...
def generate_youtube_description(
    ...
    """Generate a YouTube video description from what the film actually says —
    its narrations, or for a music video the lyrics it sings."""
    cfg = _load_cfg()
    kind = film_kind(scenes)
    content = _film_content_lines(scenes)
    if not content:
        return f"A {kind} about {title}. Generated by Stephen Spielbot."
...
def film_kind(scenes: list[dict] | None) -> str:
    """What this film IS, for human-facing copy ("documentary", "music
    video", …). Everything user-facing used to hardcode "documentary", which
    read absurd under a music video — derive it from the scenes instead."""
    modes: set[str] = set()
    singing = False
    for s in scenes or []:
        md = s.get("metadata") or {}
        singing = singing or bool(md.get("singing"))
        modes.add(str(md.get("mode") or s.get("mode") or "narration").strip().lower())
    if singing:
        return "music video"
    if modes and modes <= {"dialogue", "performance"}:
        return "acted short film"
    if modes == {"silent"}:
        return "silent short film"
    if modes & {"dialogue", "performance", "silent"}:
        return "short film"
    return "documentary"


def _film_content_lines(scenes: list[dict] | None) -> str:
    """What the film SAYS, one line per scene: the narrations, or for scenes
    that sing, their lyric slices. A music video has no narration at all, and
    the empty string used to drop its description straight to the fallback."""
    out = []
    for i, s in enumerate(scenes or []):
        text = (s.get("narration") or "").strip()
        if not text:
            text = str((s.get("metadata") or {}).get("sings") or "").replace("\n", " / ").strip()
            if text:
                text = f"(sung) {text}"
        if text:
...
def generate_youtube_tags(
    ...
    cfg = _load_cfg()
    # Narrations, or a music video's lyrics — a song film has no narration and
    # used to get NO topic tags at all.
    content = _film_content_lines(scenes)
    if not content:
        return []
