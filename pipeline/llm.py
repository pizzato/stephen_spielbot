...
# ── Cover phrase (thumbnail text) ────────────────────────────────────────────

def _clean_cover_phrase(text: str) -> str:
    """First line of the model's reply, stripped of quotes/trailing punctuation.

    Returns '' when the reply is empty or implausibly long (the model ignored
    "concise") so the caller falls back to the deterministic title-shortener.
    """
    line = (text or "").strip()
    if not line:
        return ""
    line = line.splitlines()[0].strip().strip('"').strip("'").strip().rstrip(".").strip()
    return line if 0 < len(line) <= 60 else ""


def generate_cover_phrase(title: str, phrase_style: str = "", cfg: dict | None = None) -> str:
    """Pick the punchy, concise phrase to print on the cover/thumbnail.

    The cover text shouldn't mechanically be the opening words of the title — a
    tagline after a ':' is often stronger. An LLM chooses the most clickable
    short phrase; ``phrase_style`` (a style's cover_phrase_style) steers the
    wording. ``cfg`` is the global config (for backend + keys); job configs
    strip secrets, so leave it None there and pass only ``phrase_style``.

    Falls back to the deterministic ``shorten_title_for_cover`` on any failure.
    """
    from pipeline.cover import shorten_title_for_cover
    title = (title or "").strip()
    fallback = shorten_title_for_cover(title)
    if not title:
        return fallback
    if cfg is None:
        cfg = _load_cfg()
    steer = (phrase_style or "").strip()
    style_steer = f"Preferred wording for this channel's thumbnails: {steer}." if steer else ""
    sys_msg = _prompts.system("cover_phrase")
    user_msg = _prompts.user("cover_phrase", title=title, style_steer=style_steer)
    backend = cfg.get("llm_backend", "local")
    try:
        if backend == "claude":
            api_key = cfg.get("claude_api_key", "")
        ...
        return _clean_cover_phrase(text) or fallback
    except Exception as exc:
        logger.warning("generate_cover_phrase failed: %s", exc)
        return fallback
