...
| Silent scenes — act them on H3 too | Perform each silent scene (a visual beat with no voice-over) as one H3 Ref2VA take from the portraits of whoever is on screen, instead of animating it from a first frame. The writer names a cast on each silent scene; one with nobody in it keeps the I2V path. Costs an acted scene's render time (see [Acted scenes](performance_films.md#silent-scenes-performed)) |
