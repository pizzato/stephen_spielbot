...
- **Ending** fixes a song that stops dead, by however many seconds you type:
    ...
    - **Re-generate that much longer** keeps the song and gives it a real ending: the
      take in use is kept as it is — it is not sung again — and MiniMax Music 3
      continues from the same seed for only the added seconds, finishing the words it
      cut off. If that take has no seed (generated before this), it falls back to a
      fresh longer take, and the message under the button says which one you got.
...
**Sung in this shot** (music-video beats): the lyric phrases this take mouths sit under
the soundtrack slice as an editable field — one row per phrase, with when each is heard
in seconds into the clip — alongside a summary of when any voice is heard at all. A
phrase the scene seam cuts through is already truncated to the words this window
holds (opening on "you" of "I love you" shows "you" from 0s). The prompt's song
section and its "a voice sings only from…" window are assembled from these rows, and
the burned captions follow the times, so trimming a line a scene was never meant to
sing (or nudging when one lands) is a field edit here, not a hand edit that pins the
whole prompt. Deleting every row marks the shot instrumental: the song still plays
over it, but the prompt asks nobody to sing.
