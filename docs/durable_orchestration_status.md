...
- UI controls to retry failed tasks, cancel a job, and start/stop/restart a
  worker's containers (`/api/jobs/retry`, `/api/jobs/cancel`,
  `/api/workers/control`).

## Pending

- UI controls for per-scene retry and mark worker offline/online.
...
| Browser UI smoke | n/a | Local app at `127.0.0.1:8001` | Create, Script, and Progress tabs load without console errors. |
...
- Unit tests passed with `python -m unittest tests.test_orchestrator`.
- Browser smoke test on `127.0.0.1:8001` loaded Create, Script, and Progress
